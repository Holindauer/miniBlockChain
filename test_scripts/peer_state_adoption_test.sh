...
# This test ensures that the peer ledger state adoption protocol for connecting peers works 
# The test itself contains the exact same commands as are present in the simpple_transaction_test.sh
# script. However, these commands are performed with only a single peer. Then 3 peers are connected
# and must adopt the state of the network from the sole validator. The same checks for correctness
# of ledger are applied to each of the peers.
...
# Perform checks on the correctness of the contents of the chain
for json_entry in "${blockchain_data[@]}"; do
